...
const PremiumRecommendationCard: React.FC<PremiumRecommendationCardProps> = ({
  ...
  const getSegmentIcon = (type: string) => {
    switch (type) {
      ...
      case 'Mid-Range': return <Home className="h-4 w-4" />;
  ...
  return (
    <motion.div
      ...
      className={className}
    >
      <Card className="group bg-gradient-to-br from-purple-900/20 to-purple-800/20 border border-purple-600/30 backdrop-blur-xl hover:border-purple-400/50 transition-all duration-300 overflow-hidden h-full">
        {/* Property Image */}
        <div className="relative h-48 overflow-hidden">
          ...
          <div className="absolute top-3 right-3">
            <div className={`text-3xl font-bold ${getScoreColor(property.brickMatrixScore)} bg-black/80 rounded-xl px-3 py-2 backdrop-blur-sm`}>
          ...
          <div className="absolute top-3 left-3">
            <Badge 
              ...
              style={{ backgroundColor: property.segment.color }}
            >
              {getSegmentIcon(property.segment.type)}
              <span>{property.segment.type}</span>
            </Badge>
          </div>
        ...
        <CardHeader className="pb-2">
          <div className="space-y-3">
            <CardTitle className="text-xl font-bold text-purple-100 group-hover:text-white transition-colors line-clamp-2">
              {property.title}
            </CardTitle>
